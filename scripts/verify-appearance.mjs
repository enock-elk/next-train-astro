...
assert(layout.includes('communityOn'), 'Community keyboard keeps the last full shell so the oval stays put');
assert(layout.includes('--nt-kb-lift'), 'Community keyboard lift is a measured token');
assert(layout.includes("classList.toggle('nt-community'"), 'Community tab class is stamped on html');
assert(layout.includes('html.nt-keyboard.nt-community.nt-in-app body.nav-bottom:not(.nt-immersive) #app-scroll'), 'Community keyboard lifts #app-scroll, not the oval');
assert(layout.includes('translateY(calc(0px - var(--nt-kb-lift, 0px)))'), 'Community composer lift uses --nt-kb-lift');
...
assert(!/html\.nt-keyboard[\s\S]{0,220}flex: 0 0 auto/.test(layout), 'Community keyboard does not shrink the pane to content');
...
assert(css.includes('--nt-kb-lift: 0px'), 'appearance defaults --nt-kb-lift to 0');
