...
assert(APP_VERSION === 'V9_09.06.5', `APP_VERSION is ${APP_VERSION}`);
assert(
...

{
    ...
    assert(renderer.includes('data-excl-open="1"'), 'in-app NO SVC column opens the exclusion sheet');
    assert(board.includes('[data-excl-open="1"]'), 'grid taps on a banned column open the advisory');
