...
assert(APP_VERSION === 'V9_08.28.9', `APP_VERSION ${APP_VERSION}`);
...
assert(CHANGELOG_DATA[0].id === 'V9_08.28.9' && CHANGELOG_DATA[0].features.length === 1, 'What’s New latest card is V9_08.28.9');
assert(CHANGELOG_DATA[1].id === 'V9_08.28.8', 'keep V9_08.28.8 as the previous What’s New card');
assert(CHANGELOG_DATA[2].id === 'V9_08.28.7', 'keep V9_08.28.7 as the previous What’s New card');
assert(CHANGELOG_DATA[3].id === 'V9_08.28.2', 'V9_08.28.6–28.3 folded into V9_08.28.2');
assert(CHANGELOG_DATA[4].id === 'V9_08.28.1', 'keep V9_08.28.1 as the previous production What’s New card');
...
assert(CHANGELOG_DATA[0].features.some((f) => f.includes('oval bar stays at the bottom')), 'V9_08.28.9 What’s New mentions pinned tabs');
assert(CHANGELOG_DATA[1].features.some((f) => f.includes('Max. Single Fare')), 'V9_08.28.8 What’s New mentions train-sheet fares');
assert(CHANGELOG_DATA[2].features.some((f) => f.includes('every corridor that stops there')), 'V9_08.28.7 What’s New mentions station corridors');
assert(CHANGELOG_DATA[2].features.some((f) => f.includes('Show all lines')), 'V9_08.28.7 What’s New mentions restoring the network');
assert(CHANGELOG_DATA[3].features.some((f) => f.includes('Network Lines')), 'folded 28.2 card still mentions Network Lines');
assert(CHANGELOG_DATA[3].features.some((f) => f.includes('oval bar')), 'folded 28.2 card still mentions floating tabs');
