...
assert(APP_VERSION === 'V9_08.28.2', `APP_VERSION ${APP_VERSION}`);
assert(CHANGELOG_DATA[0].id === 'V9_08.28.2' && CHANGELOG_DATA[0].features.length === 1, 'What’s New latest card is V9_08.28.2');
assert(CHANGELOG_DATA[1].id === 'V9_08.28.1', 'keep V9_08.28.1 as the previous What’s New card');
assert(CHANGELOG_DATA[2].id === 'V8_08.18.1', 'keep V8_08.18.1 in What’s New');
assert(CHANGELOG_DATA[0].features.some((f) => f.includes('board stays still')), 'V9_08.28.2 What’s New mentions Options lock');
