...
assert(APP_VERSION === 'V9_08.28.19', `APP_VERSION ${APP_VERSION}`);
...
assert(CHANGELOG_DATA[0].id === 'V9_08.28.19' && CHANGELOG_DATA[0].features.length === 3, 'What’s New latest card is V9_08.28.19');
assert(CHANGELOG_DATA[1].id === 'V9_08.28.14', 'keep V9_08.28.14 as the previous What’s New card');
assert(CHANGELOG_DATA[2].id === 'V9_08.28.13', 'keep V9_08.28.13 as the previous What’s New card');
assert(CHANGELOG_DATA[3].id === 'V9_08.28.11', 'keep V9_08.28.11 as the previous What’s New card');
assert(CHANGELOG_DATA[4].id === 'V9_08.28.10', 'keep V9_08.28.10 as the previous What’s New card');
assert(CHANGELOG_DATA[5].id === 'V9_08.28.9', 'keep V9_08.28.9 as the previous What’s New card');
assert(CHANGELOG_DATA[6].id === 'V9_08.28.8', 'keep V9_08.28.8 as the previous What’s New card');
assert(CHANGELOG_DATA[7].id === 'V9_08.28.7', 'keep V9_08.28.7 as the previous What’s New card');
assert(CHANGELOG_DATA[8].id === 'V9_08.28.2', 'V9_08.28.6–28.3 folded into V9_08.28.2');
assert(CHANGELOG_DATA[9].id === 'V9_08.28.1', 'keep V9_08.28.1 as the previous production What’s New card');
...
assert(CHANGELOG_DATA[0].features.some((f) => f.includes('Station choices open at the top')), 'V9_08.28.19 What’s New mentions planner list at the top');
assert(CHANGELOG_DATA[0].features.some((f) => f.includes('oval bar stays at the bottom')), 'V9_08.28.19 What’s New mentions pinned tabs');
assert(CHANGELOG_DATA[1].features.some((f) => f.includes('From sits in the same place')), 'V9_08.28.14 What’s New mentions From alignment');
assert(CHANGELOG_DATA[2].features.some((f) => f.includes('Check for Updates')), 'V9_08.28.13 What’s New still mentions Check for Updates');
assert(CHANGELOG_DATA[3].features.some((f) => f.includes('Station dots sit on the lines')), 'V9_08.28.11 What’s New mentions station dots');
assert(CHANGELOG_DATA[4].features.some((f) => f.includes('Park Station')), 'V9_08.28.10 What’s New mentions Park Station');
assert(CHANGELOG_DATA[5].features.some((f) => f.includes('oval bar stays at the bottom')), 'V9_08.28.9 What’s New mentions pinned tabs');
assert(CHANGELOG_DATA[6].features.some((f) => f.includes('Max. Single Fare')), 'V9_08.28.8 What’s New mentions train-sheet fares');
assert(CHANGELOG_DATA[6].features.some((f) => f.includes('Durban to Crossmoor')), 'V9_08.28.8 What’s New mentions Crossmoor train order');
assert(CHANGELOG_DATA[7].features.some((f) => f.includes('every corridor that stops there')), 'V9_08.28.7 What’s New mentions station corridors');
assert(CHANGELOG_DATA[7].features.some((f) => f.includes('Show all lines')), 'V9_08.28.7 What’s New mentions restoring the network');
assert(CHANGELOG_DATA[8].features.some((f) => f.includes('Network Lines')), 'folded 28.2 card still mentions Network Lines');
assert(CHANGELOG_DATA[8].features.some((f) => f.includes('oval bar')), 'folded 28.2 card still mentions floating tabs');
