/**
 * V8_08.18.3 polish: route-grid What’s New, version lock, keep 18.2 regressions.
 * Run: node scripts/verify-polish-08183.mjs
 */
...
if (APP_VERSION !== 'V8_08.18.3') fail(`APP_VERSION is ${APP_VERSION}`);
const latest = CHANGELOG_DATA[0];
if (latest?.id !== 'V8_08.18.3') fail(`CHANGELOG_DATA[0].id is ${latest?.id}`);
if (!Array.isArray(latest?.features) || latest.features.length < 1) {
    fail(`What's New must have at least one bullet, got ${latest?.features?.length}`);
}
...
if (/admin|dev hub|telemetry|firebase|dump|seo|googlebot/i.test(wn)) {
    fail('Whats New must stay commuter-only');
}

const pkg = JSON.parse(readFileSync('package.json', 'utf8'));
if (pkg.version !== '8.8.18.3') fail(`package.json version is ${pkg.version}`);
const appVer = JSON.parse(readFileSync('public/app-version.json', 'utf8'));
if (appVer.version !== 'V8_08.18.3') fail(`app-version.json is ${appVer.version}`);
...
if (failures.length) {
    console.error(`\n✗ polish 08183 failed (${failures.length}):`);
    for (const f of failures) console.error(`  - ${f}`);
    process.exit(1);
}
console.log('✓ V8_08.18.3 polish (route grids, version, 18.2 regressions)');
