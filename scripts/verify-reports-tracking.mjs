...
assert(mapAppSource.includes('interpolateRideMarkerLatLng'), 'remote map marker still uses the lab place-on-ping helper');
assert(!mapAppSource.includes('interpolateAlongRidePath'), 'trains do not interpolate along the painted rail');
assert(!mapAppSource.includes('STATION_APPROACH_M'), 'station-approach glide is gone');
assert(!mapAppSource.includes('STATION_DWELL_SEC'), 'station-dwell glide is gone');
assert(!mapAppSource.includes('snapTrainToRail'), 'map no longer re-snaps pings onto the painted corridor');
assert(mapAppSource.includes('applyTrainGlyphYaw'), 'oval still yaws from the collected ping bearing');
assert(mapAppSource.includes('offsetLatLngByBearing'), 'opposing trains offset from the real ping, not a painted-path slide');
assert(ridePingsSource.includes('scoreFixToRailPath'), 'onboard collection still snaps GPS to the train rail path');
assert(ridePingsSource.includes('alignBearingToJourney(snap.trackBearing, journeyH)'), 'collected bearing is rail tangent aligned to the journey');
assert(mapAppSource.includes('Do not glide along painted rail'), 'markers sit on the last projected GPS ping');
...
assert(mapAppSource.includes("ping: Object.assign({}, newest"), 'train click sends the collected ping into the tracking card');
