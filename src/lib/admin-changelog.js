...
export const ADMIN_CHANGELOG = {
    'V9_09.18.4': [
        'CleverAds top units dock into #nt-ad-scroll-host (first child of #app-scroll) before the iframe loads, so they sit in the Next Train frame and scroll with the board. Scroll-away is not dismiss; the vendor X is not required (cracked-screen safe). Loaded iframes are never reparented. Already-painted stickies follow #app-scroll via CSS translate plus a host spacer; undocked wrappers use pointer-events:none so a full-viewport overlay cannot trap scroll. #main-content is not translateY-pinned. Occupancy still needs a loaded creative; empty leftovers collapse. What’s New is the ad scroll bullet. forceShow stays false. FORCE_UPDATE_REQUIRED stays true.',
    ],
    'V9_09.18.3': [
